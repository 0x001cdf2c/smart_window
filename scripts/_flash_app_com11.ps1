...
$python = "$toolsPath\python\v5.5.4\venv\Scripts\python.exe"
$idfpy = "$env:IDF_PATH\tools\idf.py"

Set-Location 'C:\Users\28145\Desktop\IOT\code\Net'

Write-Host "=== Building (app only) ==="
& $python $idfpy build 2>&1 | Select-Object -Last 40
if ($LASTEXITCODE -ne 0) { Write-Host "Build failed"; exit $LASTEXITCODE }

Write-Host ""
Write-Host "=== Flashing app to COM11 (preserve SR model partition) ==="
& $python $idfpy -p COM11 app-flash 2>&1 | Select-Object -Last 20
if ($LASTEXITCODE -ne 0) { Write-Host "Flash failed"; exit $LASTEXITCODE }
