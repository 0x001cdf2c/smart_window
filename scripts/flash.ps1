...
Write-Host "=== Flashing to COM8 (ESP32-P4) ==="
& $python $idfpy -p COM8 flash 2>&1 | Select-Object -Last 15
if ($LASTEXITCODE -ne 0) { Write-Host "Flash failed"; exit $LASTEXITCODE }

Write-Host ""
Write-Host "=== Flash complete! Starting serial monitor ==="
& $python $idfpy -p COM8 monitor
